## App.jsx: review 7/10 → hooks useJoinSession + useAuth (2026-09-26)
- Avaliação 1–10 pedida → plano completo aprovado e executado. 6 ficheiros: `App.jsx` (593→~230 linhas, só composição), **novos** `hooks/useJoinSession.js` (me/sessão/joining/erro, listeners, retry 10s×5, auto-join, joinRoom/switchRoom/restoreSession), `hooks/useAuth.js` (nome/token/fase/modo/saves, authenticate devolve boolean), `utils/joinErrors.js` (isAuth/isTransient/isRoomUnavailable) + `joinErrors.test.mjs` (`test:joinerrors`, 17/17), `package.json`.
- 28 props da `LandingPage` viram um spread (API dela intacta); `roomCode` vive no `App` (único estado partilhado pelos hooks); `onRoomGone` repõe `authPhase="mode"` sem dependência circular. Fixes de passagem: `JOIN_TIMEOUT_MS`/`MAX_JOIN_RETRIES` nomeados, `AbortController` no fetch dos saves, 2 `console.log` apagados (mantido o `warn` do retry).
- Checks: `test:joinerrors` 17/17 · eslint limpo nos tocados · `check:types` OK. `build` falha em 5 `UNRESOLVED_IMPORT` no `GameHeader.jsx` — pré-existente (confirmado via stash, WIP alheio, fora do plano). Sem mudança visual → sem mobile-resp-check; sem servidor → sem audits.
...
- Cor por tipo, não por string: payload passa `isGoal: e.type === "penalty_goal"` (antes comparava `result === "GOLO!!!"` — quebrava se o texto do servidor mudasse). A11y: `role="status" aria-live="assertive"` no cartão. `animate-bounce` só durante o suspense (parado na revelação). REVEAL 2000→1500ms: resultado legível 1,5s em vez de 1s (CLEAR mantém 3s, ritmo do jogo intacto).
- Lista vazia → `[]` (apanhado pela regressão S1: o reveal órfão a 0ms). S1–S5: 0 eventos, 1 penálti, 2 escalonados + 1 reveal, nulls filtrados, cor por tipo.
- Checks: `test:penaltysuspense` 5/5 · `check:types` OK · `lint` limpo nos tocados (2 erros pré-existentes globais, confirmados via stash). Sem mudança estrutural de layout → sem mobile-resp-check. Client-side only → audits de servidor N/A (não há sala ativa).
...
- F2: cartões duplicados mobile/desktop extraídos e a consumir contexto diretamente: `MoraleCard` (variante `glow` = desktop), `MentalityChips`/`MentalityPills` com `STYLE_ORDER`/`STYLE_META` partilhados, `FormationCard` único (header + grelha; blurb e `data-tour` só desktop via props).
- F3: lógica 🟥/🩹/✈️ unificada em `unavailableMark` — PlayerRow e campo partilham a fonte (divergência antiga: campo mostrava ✈️ no caso degenerado sem datas, agora 🩹(neg) como as linhas).
- F4: `Pitch` (IIFE ~200 linhas) passa a componente nomeado que consome `useTactics()`/`useGame()`; `StatusPicker` também consume o contexto (cai prop-drilling de 5 props; continua fora de TacticsView — garantia anti-remount mantida); `getBestForFormation` para o módulo.
- F5: indentação do ternário do espectador da Taça corrigida.
- YAGNI: sem useMemo/useCallback novos, sem ficheiros novos, API do TacticsContext intacta.
- Checks: `lint` limpo no ficheiro (globais: os 2 erros pré-existentes) · `check:types` OK · `test:mobile` 160/165 + `test:mobile:landscape` 192/198 — falhas = baseline conhecida do `roomhub-resp-test` (já registada na entrada LeagueStandings; tactics 5/5 e 3/3 PASS) · screenshots vistos (360 portrait, 667 landscape). Desktop xl sem harness — classes transplantadas verbatim.

## Limpeza de ficheiros obsoletos (2026-09-26)
- Levantamento (só leitura) + deleção de obsoletos, aprovado pelo utilizador (🗑️ + ⚠️; 💾 locais e `NOTES.md` fora do âmbito). Apagados do git: `block.txt` (snippet JSX de 643 linhas commitado por engano — `FILES.md` documentava-o como "snippet do `LandingPage`"), `server/gen-fixture.mjs` (debug one-off; importava `better-sqlite3` ausente das deps → não corria), `server/scripts/economySim.mjs` (calibragem one-off, 0 refs; a constante já está no código), `COMO_SIMULA_JOGOS.md` + `docs/ENGINE_AUDIT_PLAN.md` (0 refs), `FILES.md` (inventário desatualizado — listava 4 componentes já apagados), `docs/FLOW_JOGO.md` + `docs/FLUXO-JOGO.md` (docs de fluxo sobrepostos), `.opencode/` (config do agente OpenCode, substituída por `.pi/`), `.superpowers/` + `docs/superpowers/` (artefactos SDD 2026-09-03). Fora do git: `koboldcpp` (binário de 487MB, untracked).
- Verificação antes de apagar: 0 referências em código (só self-refs) e 0 em docs canónicos (AGENTS/CLAUDE/README/STYLE); as menções em `NOTES.md` são registos históricos (memória) e mantêm-se. `server typecheck` exit 0.
- Mantidos (verificados em uso): todos os `client/scripts/*.mjs`, harnesses `*-resp-test.*` (suite mobile), `server/scripts/*Regression.mts` (21), `fetchZerozeroLogos.ts`, `backupDatabases.js` (gera `backups/`), `engineCalibration.mts`, `client/public/sw.js`.

## LeagueStandings: review 7,5/10 → correções (2026-09-26)
- Avaliação 1–10 pedida → plano aprovado (incl. os itens "skipados" — memoização, `<details>`, ícone `▶`, unificar iniciais; virtualização ficou de fora: ~80 linhas, quebraria o stagger e o harness). 5 ficheiros: `LeagueStandings.jsx`, `LiveStandings.jsx`, `ShowcaseSections.jsx`, **novo** `shared/FormDots.jsx`, **novo** `utils/initials.js`.
- DRY: sort inline do `DivisionTable` era cópia byte-a-byte de `compareStandingsRows` (`rankStandings`) — o ficheiro importava o util mas só o usava para `prevStandings`; agora usa-o para tudo (uma só fonte de ordem vs servidor). `TOTAL_MATCHWEEKS = 14` local → `SEASON_JORNADAS` de constants; `DIVISION_NAMES` local idem (duplicado de constants). `FormDots` existia em 3 cópias (LeagueStandings/LiveStandings/ShowcaseSections) → partilhado com `size` sm/md + `role="img"`/`aria-label` (dots eram só cor). `getInitials` (2 primeiras iniciais, sem strip de acentos) unificado com o `coachInitials` do LiveStandings (primeira+última, acentos removidos) em `utils/initials.js` — os selos de treinador da tabela agora mostram primeira+última ("AC" em vez de "AJ" no caso longo do harness).
- A11y/UX: linhas da tabela eram `onClick` sem teclado → `tabIndex={0}` + Enter/Espaço no `motion.tr`; `MatchweekRow` de `useState`+botão+`▶` para `<details>/<summary>` nativo com `chevron_right` `group-open:rotate-90`; emojis 📋⚽🏆 → `material-symbols-outlined` (`receipt_long`/`sports_soccer`/`emoji_events`); estado vazio sem equipas → `EmptyState` ("Época ainda não iniciada").
- Memoização: `useMemo` em `divTeams`/`prevIndex` (DivisionTable), `teamMap` (MatchweekResults), `humanTeamIds`/`coachNames` (LeagueStandings); `FormDots` com `memo`. `teamMap` chegou a ficar após o early-return `!matchweeks.length` durante o refactor (violação de hooks) — apanhado no lint seguinte, movido para cima.
- Mid-refactor partiu o ficheiro 2× (uso de `TOTAL_MATCHWEEKS`/`useState` após apagar as defs; fecho de tags do `<details>` mal trocado) — apanhado por grep+lint antes de commitar.
- Checks: `lint` limpo nos tocados (globais: 2 erros pré-existentes `landing-resp-test`/`GameContext`) · `check:types` OK · portrait 160/165 + landscape 192/198 — as falhas são o `roomhub-resp-test` pré-existente (5+6, precisa `GameProvider`; confirmado via stash: falha igual sem as mudanças). `standings-resp-test` PASS nas duas passagens (320 tem `clipEls` dos `overflow-x-auto` intencionais).

## AdminPanel: review 7,9/10 → correções (2026-09-26)
- Avaliação 1–10 pedida → plano aprovado (seleção por nome, early-return, a11y) e executado. Só `AdminPanel.jsx` + `Button.jsx`.
...
- Badges `unreadRoom/unreadGlobal` nas tabs Sala/Global (`setUnread*` agora exportados do contexto); limpeza passa a ser por canal visível no RoomHub em vez de tudo-ao-abrir no `GameContext` (o outro canal mantém o badge).
- Tema/a11y/perf: `#1e1e1e`/`#111` → `bg-surface-container(-low)` (STYLE.md); `role="dialog"` + `aria-label` + fechar com `Esc` + `aria-label` no input/botão; `STATUS` estático, `Map`/`Set` em `useMemo` (presença, membros, coaches, equipas), `useCallback` nos envios.
- Checks: `check:types`/JSDoc OK; `lint` só os 2 erros pré-existentes (confirmados via `git stash`); sem `test:mobile` (tweaks pontuais, sem mudança estrutural).
## CalendarioTab: review 6,5/10 → correções (2026-09-26)
- Avaliação 1–10 pedida → plano completo aprovado e executado com ponto 5 incluído. Só `client/src/views/CalendarioTab.jsx` (reescrita, mesma estrutura visual): builders puros fora do render (`buildFriendly/Cup/LeagueItem` + `findMyFixture`/`opponentOf`/`splitScore`, cobrem os dois formatos de chaves home/away), `useMemo` em `myTeam`/`myDivTeams`/`eliminatedCupRound`/`calEntries`, `TeamCircle` com `memo` fora do componente, `ScoreBlock` extraído.
- Partilhados (§10): vazios/loading passam a `EmptyState`, lista em `Panel` («Jogos», `meta` com contagem), chips de desfecho/estado/local em `Badge` (`sold`/`warning`/`error`/`info`/`neutral`); `rounded-lg`→`rounded-md`, `space-y-5`→`space-y-4`, `tabular-nums` nos marcadores, emojis ⚽🤝🏆→`material-symbols-outlined`, `VS`→`vs`.
...
- `cupFlowHelpers` (reset de época) lê `fansBaseByDivision` das constantes — sem mudança própria. `schema.sql` + `ALTER TABLE` de BDs antigas: default 60→30.
...
## TeamHistoryView — fixes + lista de jogos + filtros (2026-09-26)
- Servidor (`requestClubHistory` em `socketSessionHandlers.ts`): payload `clubHistoryData` passa a incluir `games` — jogos de Liga (de `matches`) + Taça e amigável (de `cup_matches`, ronda 0 → `kind: "friendly"`); só épocas concluídas e `played = 1`, com nomes dos clubes via JOIN e `roundName` do servidor (`CUP_ROUND_NAMES`/`FRIENDLY_ROUND_NAME`). Verificação: Taça e amigáveis já apareciam no `CalendarioTab` (filtro Taça inclui ronda 0); no histórico só existiam como troféu no Palmarés (`Vencedor da Taça de Portugal`) — agora há lista completa de jogos.
- Cliente (`TeamHistoryView.jsx`): fixes — `isBest` por identidade de objeto (não `rec.season === bestSeason.season`), ternário redundante do `subtitle` simplificado, `key` estável nos troféus, fallback do emblema com `useState` (sem manipular DOM no `onError`); cabeçalho sempre visível (zeros/`—` quando vazio, estado vazio abaixo); nova secção Jogos com `TabBar expand` Todos/Liga/Taça/Amigáveis (marcador na perspetiva do clube, `gp` na Taça); filtro na timeline Todos/Mercado/Treinadores/Prémios (só frontend).
- Mobile: `TabBar` sem wrap rebentava 320–390px (overflow até 94px) → `expand` + `w-full sm:w-auto`; brilho do cabeçalho (`-left-16 w-80` com blur) alargava o `scrollWidth` → `left-0 w-full`. Novo harness `teamhistory-resp-test.{html,jsx}` (regression asset): portrait 5/5, landscape 6/6 PASS, `pageErr=0`.
- Checks: server typecheck OK; `audit:socketio` 0 erros (97 warnings pré-existentes); SQL novo validado com `EXPLAIN QUERY PLAN` (sem saves com jogos realizados para teste ponta-a-ponta); client lint só os 2 erros pré-existentes (confirmado via `git stash`); `check:types`/JSDoc OK.
...
## Taça — CupTab + CupBracketPage para STYLE.md (2026-09-26)
