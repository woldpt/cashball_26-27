...
- Fadiga em jogo dos GR reduzida (`fatigueGRSkipBonus: 0.5` em `MATCH_TUNING`): os GR (posição `"GR"`, incl. GR improvisado via clone) somam +0.50 ao `skipChance` do rolo de cansaço, em vez da fórmula pura `(resistência − 1) × 0.00816` — cansam-se muito menos que jogadores de campo (res. média 25: ~4.8 → ~1.8 skill/90min). Helper `fatigueSkipChance(p)` em `server/game/engine.ts` partilhado por `trackFatigue` (progressiva, 15 min) e `applyFatigue` (min 60 em frio/neve). Verificação: typecheck ok, audit:socketio 0 erros, audit:gamestate 445WU8 0/0/0.
...
- `TacticsView`: fundo ténue por posição em cada linha de jogador — `POS_COLORS` ganhou campo `soft` (hex @ 8% de alpha); `PlayerRow` define `style={{ "--pos-soft": pos.soft }}` + classe base `bg-[var(--pos-soft)]` (valor dinâmico via CSS var). Estado drag-over mantém o ring (`bg-white/5` tirado do ramo para não competir com a classe base pela mesma propriedade); `hover:bg-white/5` continua (variant ganha à classe base). Verificação: lint + check:types ok; build local bloqueado por bindings nativos em falta (rolldown/lightningcss, pré-existente) — CSS final a confirmar no `docker compose up --build` do utilizador.
...
- `StadiumIllustration` estética: reescrito com perspectiva de transmissão — relvado em fuga (faixas horizontais em foreshortening, baliza única com emolduramento + rede, círculo central elíptico), sombra das bancadas projetada no relvado, end caps inclinados com shading derivado da cor da equipa, fileiras de assentos + multidão em manchas com jitter, pala em arco com nervuras e testadeira na cor do clube, portão principal central + portões arquivados, sol com halo, nuvens desfocadas, serra em duas camadas, pinheiros assentes no horizonte e vignette subtil. Props e viewBox 800×272 inalterados; overlay das views intocado. Novo harness `stadium-resp-test` (4 escalotes + cores extremas) como ativo de regressão. Verificação: lint + check:types ok; mobile portrait 125/125 + landscape 150/150 PASS; screenshots 390/736 ok.

- Fotos treinadores D1 via TUI (só fotoTreinador, 12/12 OK, audit 0/0/0): Rui Borges 28061, Farioli 49791, Marco Silva 4199, etc. Nota: Marítimo/Juventude já estavam no repo desde c3bced7 — se não aparecem numa sala, é porque a sala tem nome de treinador antigo (backfill é por nome); salas novas trazem tudo. Commit por fazer push.
...
- `GameLayout`: removido `lg:pb-12` (48px de padding inferior morto no desktop — não há nav inferior em `lg`, a sidebar é lateral); agora `lg:pb-0`.- `StadiumIllustration`: formato baixo (800×272) — torres de luz encurtadas e relvado estendido em primeiro plano para as faixas apanharem bancada + balizas + relvado; prova visual em PNG dos 4 escalões antes de fechar. Verificação: lint + check:types ok, mobile portrait 125/125 + landscape 150/150 PASS.
...
- Foto real dos treinadores: `CoachAvatar` ganhou prop `photo` (prioridade enviada > real > procedural, flags de erro separadas para fallback em cascata). `TeamSquadView` passa `selectedTeam.coach_photo`. `CoachMarketModal` (Mercado de Treinadores) mostra a foto em cada linha — para isso `CoachMarketEvent` ganhou `coachPhoto`, preenchido no servidor em `recordMarketEvent` (agora async, procura `managers.photo` por nome, para persistir mesmo quando o treinador despedido fica desempregado). `GameLayout` passa `meName`/`coachAvatars`/`backendUrl` ao modal. Verificação: typecheck + lint + check:types ok, mobile portrait 125/125 + landscape 150/150 PASS; prova visual em harness temporário (foto real do Mitchell/Bruno, procedural nos sem foto).
