...
- **WaitingCoachesModal já não cresce com o chat (fix):** causa raiz — cartão com `max-h-full` sem efeito (pai `overflow-y-auto` de altura automática) + contentor interior sem limite + bolhas sem quebra de palavra + `45vh`+`45vh` em mobile empilhado. Fix só em classes: cartão `max-h-[90dvh]`, interior `flex-1 min-h-0`, lista `max-h-[24vh]`, chat `flex-none min-h-[10rem] max-h-[34vh]` (+ `self-stretch` em lado-a-lado), bolhas `break-words [overflow-wrap:anywhere]`, `min-w-0`/`overflow-hidden` nos flex. Checks: lint + check:types OK. mobile-resp-check NÃO correu — `vite dev` não arranca neste ambiente (binding nativo `@rolldown/binding-linux-x64-gnu` em falta, `npm install` sem permissões); mudança limitada a um modal sem harness, sem impacto noutros componentes.
- **Final da Taça — emblemas duplicados (fundido num só bloco):** o `CupFinalStage` mostrava o frente-a-frente grande (crest `lg` + nome + treinador) e o `LiveMatchHero` repetia tudo na barra broadcast (crest `sm` + nome + treinador). Opção do utilizador: fundir — o centro `VS` do frente-a-frente passou a botão de marcador (golos casa:fora com flash de golo, minuto + fase, `onScoreClick`, respeita `readOnly`/`isPlayingMatch`); o hero na final renderiza com `hideScoreboard` (nova prop que oculta meta strip + barra broadcast, mantendo info strip, pausa de substituição, Jogador do Jogo, cronómetro, eventos, comentário e `GoalFlashOverlay`). Checks: lint + check:types OK. mobile-resp-check NÃO correu — mesmo bloqueio de ambiente já registado (binding `rolldown` gnu em falta, `node_modules` de root sem permissão de escrita); rótulo do minuto sem `nowrap` para não rebentar a 320px.
