...
function ownGoalPhrase(name: string): string {
  return pickPhrase([
    ...
    `${name} quis ser herói no corte e saiu vilão. Auto-golo!`,
...
function defaultGoalPhrases(name: string): string[] {
  return [
    ...
    `${name} faz parecer fácil. Alerta: não é.`,
...
function penaltyMissPhrase(name: string, missType: string): string {
  const pools: Record<string, string[]> = {
    ...
    "AO POSTE!": [
      ...
      `Toc! O ferro. ${name} atirou para o lado errado da barra. Por centímetros.`,
      `${name} mandou ao poste. A bola saiu. O desespero ficou.`,
      `Que azar de ${name}! A bola bateu na barra e saiu. O universo disse não.`,
      `Bateu no ferro e saiu. O estádio inteiro sentiu o baque de ${name}.`,
      `${name} acertou em cheio… na barra. Detalhe cruel.`,
    ...
    "AO LADO!": [
      `Ao lado! ${name} mandou para as bancadas. Os adeptos nem queriam ver.`,
      `Fora! ${name} deu uma aula de como não bater um penálti.`,
      `${name} rematou para a bancada. Literalmente.`,
...
function varPhrase(name: string): string {
  return pickPhrase([
    ...
    `Fora de jogo por meia bota. O VAR esticou a régua e ${name} encolheu.`,
...
function yellowPhrase(name: string): string {
  return pickPhrase([
    ...
    `${name} vê cartão amarelo. O árbitro avisa: calma, rapaz.`,
    ...
    `Toque a mais em ${name}. O árbitro assinalou falta e exibiu o cartão amarelo. Óbvio.`,
    `${name} jogou duro demais. O árbitro aproximou-se e sacou o cartão do bolso.`,
    ...
    `O árbitro saca do bolso e ${name} não escapa.`,
    ...
    `${name} puxa o avançado para o relvado com estilo… de falta. Amarelo merecido.`,
...
function redPhrase(name: string): string {
  return pickPhrase([
    ...
    `Cartão vermelho! ${name} não vai assistir ao resto. Talvez seja melhor assim.`,
    ...
    `Vermelho directo! ${name} nem chegou a fechar o fecho da mala.`,
...
function emergencyGkPhrase(name: string): string {
  return pickPhrase([
    ...
    `${name} vai para a baliza e já pede desculpa pelos primeiros remates.`,
    `Missão de emergência: ${name} calça as luvas e torce por defesas impossíveis.`,
    `${name} assume a baliza. O treinador espera que os adversários falhem.`,
    `Plano B em ação: ${name} defende a baliza como pode e reza.`,
    `${name} vai para o lugar que ninguém quer no último minuto — e vai.`,
    `Luvas emprestadas, coração grande: ${name} fica na baliza.`,
    `A equipa perdeu um guarda-redes e ganhou um desbravador à baliza: ${name}.`,
...
function subPhrase(outName: string, inName: string): string {
  return pickPhrase([
    ...
    `${outName} sai a aplaudir-se a si próprio. ${inName} entra a aplaudir o banco.`,
    `O treinador gasta o papel: ${outName} sai, ${inName} entra.`,
    `${outName} vai para a bancada tomar uma água. ${inName} vem fazer a diferença.`,
    `Mudança: ${inName} entra com energia, ${outName} vai para o banco dos heróis.`,
    `${outName} cumpriu o seu tempo de bateria. ${inName} entra carregado.`,
    `A equipa muda de pele: ${inName} dentro, ${outName} para o banco.`,
    `${inName} entra para ser solução; ${outName} sai como parte da estatística.`,
    `Saem as pernas cansadas de ${outName}, entram as pernas novas de ${inName}.`,
    `O treinador mexe no tabuleiro: ${inName} entra no lugar de ${outName}.`,
    ...
    `O treinador gasta uma carta: sai ${outName}, entra ${inName}.`,
...
function chanceSavedPhrase(attacker: string, grName: string): string {
  return pickPhrase([
    ...
    `Cabeçada de ${attacker} ao centro — ${grName} agarra sem suar.`,
    `${attacker} remata de longe com o pé direito — ${grName} encaixa no segundo balão.`,
    `${grName} defende o livre de ${attacker} como quem faz compras de rotina.`,
    `Contra-ataque veloz, ${attacker} a solo... ${grName} fecha a porta a tempo.`,
    `${attacker} tenta o chapéu a ${grName} — ${grName} recua e aguenta firme.`,
    `Apanha de primeira de ${attacker} em grande ângulo — ${grName} estica e salva a honra.`,
    `Cruzamento rasteiro, ${attacker} empurra... ${grName} atira-se a pontapé e segura.`,
    `${attacker} remata no fim do contra-ataque — ${grName} defende com os dedos e respira.`,
    `O golo estava à vista de ${attacker}. ${grName} tinha um plano: os punhos.`,
    `${attacker} remata com veneno do interior da área — ${grName} desvia com raiva.`,
    `${grName} lê o remate de ${attacker} antes do relógio e agarra no primeiro tempo.`,
    `Bola alta na área, ${attacker} aparece... e ${grName} também. Defendido.`,
    `${attacker} bate o canto em arco — ${grName} corta ao segundo poste. Susto.`,
...
function chancePostPhrase(attacker: string): string {
  return pickPhrase([
    ...
    `${attacker} remata com força — o poste devolveu. Quase foi golo.`,
    `O ferro traiu ${attacker}: a bola bateu no poste e saiu para fora.`,
    `Cabeçada de ${attacker} a rasar o poste — a baliza fez barulho e nada mais.`,
    `${attacker} dispara de primeira no coração da área... e o poste respondeu primeiro.`,
    `Que raiva! ${attacker} rematou em jeito, o poste segurou e soltou por fora.`,
    `A bola de ${attacker} tocou o ferro e fugiu. Sorte do adversário, antes.`,
    `Livre de ${attacker} por cima da barreira — e directo ao poste.`,
    `${attacker} remata de longe com a raiva do mundo — e o poste não perdoa.`,
...
function chanceOffTargetPhrase(attacker: string): string {
  return pickPhrase([
    ...
    `Cabeçada de ${attacker} por cima da barra — tinha tudo para entrar.`,
    `${attacker} remata de longe com vontade — e passa ao lado da baliza.`,
    `Apanha de primeira de ${attacker}... para fora do estádio. Quase.`,
    `${attacker} tenta o chapéu ao guarda-redes — sai a rasar o relvado, não a rasar o poste.`,
    `Contra-ataque de ${attacker} e remate ao lado — a defesa respirou fundo.`,
    `Livre de ${attacker} por cima da barreira — e por cima da baliza também.`,
    `${attacker} remata com o pé esquerdo e o alvo agradece.`,
    `Cruzamento de ${attacker} na cabeça... e a bola vai parar à bancada.`,
    `${attacker} atirou para o escuro. Nem o guarda-redes viu.`,
...
function nearMissPhrase(name: string): string {
  return pickPhrase([
    ...
    `${name} atirou com tudo… e foi por cima da barra. A baliza ficou intacta mas tremia.`,
    ...
    `A bola de ${name} bateu na barra e voltou para o campo. O ferro hoje está do outro lado.`,
    ...
    `${name} remata… e passa a centímetros da barra. Estava tão perto.`,
    ...
    `A barra diz não a ${name}. O ferro salvou a equipa adversária.`,
...
export function bettingPhrase(
  ...
  return pickPhrase([
    ...
    `Bola ao centro e olhos nas odds: ${odds.home} (1) · ${odds.draw} (X) · ${odds.away} (2). ${homeName} frente a ${awayName}.`,
    `Previsão das casas de apostas antes do pontapé de saída — ${homeName} ${odds.home}, X ${odds.draw}, ${awayName} ${odds.away}.`,
    ...
    `O balcão de apostas está cheio: ${homeName} ${odds.home}, empate ${odds.draw}, ${awayName} ${odds.away}.`,
    `Fiche de quem tem coragem: ${homeName} paga ${odds.home}, X paga ${odds.draw}, ${awayName} paga ${odds.away}.`,
    `A banca anda a fazer contas: ${homeName} ${odds.home} · X ${odds.draw} · ${awayName} ${odds.away}.`,
    `Mercado aberto antes do apito: ${homeName} ${odds.home}, empate ${odds.draw}, ${awayName} ${odds.away}.`,
    `Os números não mentem, dizem as casas: ${homeName} ${odds.home} · ${odds.draw} · ${awayName} ${odds.away}.`,
    `Quem arrisca antes do jogo: ${homeName} ${odds.home}, X ${odds.draw}, ${awayName} ${odds.away}.`,
    `Apostas fecham ao apito: ${homeName} paga ${odds.home}, empate ${odds.draw}, ${awayName} paga ${odds.away}.`,
