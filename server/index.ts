			...
			(err: any) => {
				...
				if (managerName) {
					...
					db.get(
						...
						(err: any, row: any) => {
							if (!err && row)
								rows.push({ key: "teamName", value: row.teamName });
							pendingQueries--;
							if (pendingQueries === 0) finish();
						},
					);
...
app.get("/auth/manager-info", async (req, res) => {
	try {
		...
		return res.json({
			...
			birthYear: result.info.birthYear,
			rooms,
		});
	} catch (error) {
...
io.on("connection", (socket) => {
	...
	socket.on(
		"setTrainingFocus",
		(trainingFocus: string, callback?: (ok: boolean) => void) => {
	...
	socket.on(
		"getTrainingFocus",
		async (callback: (focus: string | null) => void) => {
			const game = getGameBySocket(socket.id);
			const player = game ? getPlayerBySocket(game, socket.id) : null;
