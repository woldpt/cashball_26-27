...
export function SkillBadge({
  ...
  const sm = size === "sm";
  const skillCls = sm ? "text-[14px]" : "text-[16px] sm:text-[17px]";
  ...
  return (
    ...
    >
      <div
        className={`flex flex-col items-center justify-center gap-0.5 bg-amber-400/12 border-r border-amber-400/20 ${cellCls}`}
      >
        <span className="text-[7px] uppercase tracking-widest text-amber-200/70 font-bold leading-none">
          Skill
        </span>
        <span className="flex items-center gap-1 leading-none">
          ...
          <span
            ...
            style={{ textShadow: "0 0 12px rgba(251,191,36,0.55), 0 0 22px rgba(251,191,36,0.22)" }}
          >
            {skill ?? "—"}
          </span>
        </span>
