...
export function Panel({
  ...
  return (
    <section className={`bg-surface-container rounded-md overflow-hidden ${className}`}>
      <div
        className={`px-5 py-4 flex items-center justify-between bg-surface-container-high/50 ${headerClassName}`}
      >
        <h2
          className={`text-base font-black font-headline tracking-tight text-tertiary uppercase flex items-center gap-2 ${titleClassName}`}
        >
          {icon && (
      ...
      <div className={`${padded ? "p-3 md:p-4" : ""} ${bodyClassName}`}>
        {children}
      </div>
    </section>
  );
