...
/**
 * Compact match player card that expands on hover to show resistance + form.
 *
 * Compact:  [bar] [POS] Nome ★                   42
 * Expanded: [bar] [POS] Nome ★   42  │ RES 4 │ FORM 💪
 *
 * Props:
 *   player      — player object (name, position, skill, is_star, resistance, form)
 *   posStyle    — result of getPosStyle(position)
 *   selected    — is this player selected for swap?
 *   disabled    — is this player unavailable?
 *   selectable  — can this player be clicked to select?
 *   onPick      — click handler
 *   title       — tooltip text
 *   swapIndicator — show ↔ arrow for halftime swap
 */
export function MatchPlayerCard({
  ...
  return (
    ...
    >
      ...
      {!expanded && (
        <span
          className={`shrink-0 text-[11px] font-black tabular-nums mr-2 ${
            selected ? "text-rose-300" : "text-on-surface-variant/80"
          }`}
        >
          {player.skill ?? "—"}
        </span>
      ...
      {/* Expanded: skill + resistance + form */}
      {expanded && (
        <>
